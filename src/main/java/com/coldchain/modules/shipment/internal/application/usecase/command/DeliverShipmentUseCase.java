...
public class DeliverShipmentUseCase {
    ...
    public DeliverShipmentUseCase(ShipmentRepository shipments, CustodyLog custodyLog, ShipmentApiMapper mapper, ApplicationEventPublisher events,
    ...
    public ShipmentResult execute(UUID shipmentId) {
        ...
        Shipment moved;
        try {
            moved = shipments.save(shipment.deliver(now));
        } catch (IllegalStateException refused) {
            throw DomainException.of(ShipmentErrorCode.ILLEGAL_TRANSITION, refused.getMessage());
        }
        custodyLog.append(moved.id(), CustodyEventKind.DELIVERED, null,
